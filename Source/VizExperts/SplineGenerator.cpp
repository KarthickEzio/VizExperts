...
void ASplineGenerator::GenerateComponents(USplineComponent* spline)
{
	...
	for (int i = 0;i <= points.Num()-1;i++) {
		spline->AddSplinePointAtIndex(points[i], i, ESplineCoordinateSpace::Local, true);
	}
	...
	if (showMesh && segmentLength > 1)
	{
		...
		for (int32 i = 0; i < segs.Num(); i++)
		{
			...
			DynamicMatierial->SetTextureParameterValue(FName(TEXT("Texture")), texture);
			splineMesh->SetMaterial(0, DynamicMatierial);
			splineMesh->SetWorldScale3D(FVector(1.0, 1.0*width, 1.0 * width));
